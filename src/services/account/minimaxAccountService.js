...
class MinimaxAccountService {
  constructor() {
    // Redis键前缀
    this.ACCOUNT_KEY_PREFIX = 'minimax_account:'
    this.SHARED_ACCOUNTS_KEY = 'shared_minimax_accounts'

    // 使用 commonHelper 的加密器
    this._encryptor = createEncryptor('minimax-account-salt')
    ...
    setInterval(
      () => {
        ...
        logger.info(
          '🧹 MiniMax account decrypt cache cleanup completed',
  ...
  async createAccount(options = {}) {
    ...
    // 验证必填字段
    if (!apiUrl || !apiKey) {
      throw new Error('API URL and API Key are required for MiniMax account')
    }
    ...
    logger.debug(
      `[DEBUG] Saving MiniMax account data to Redis with key: ${this.ACCOUNT_KEY_PREFIX}${accountId}`
    )
    logger.debug(`[DEBUG] MiniMax Account data to save: ${JSON.stringify(accountData, null, 2)}`)

    await client.hset(`${this.ACCOUNT_KEY_PREFIX}${accountId}`, accountData)
    await redis.addToIndex('minimax_account:index', accountId)
  ...
  async getAccount(accountId) {
    const client = redis.getClientSafe()
    logger.debug(`[DEBUG] Getting MiniMax account data for ID: ${accountId}`)
    const accountData = await client.hgetall(`${this.ACCOUNT_KEY_PREFIX}${accountId}`)

    if (!accountData || Object.keys(accountData).length === 0) {
      logger.debug(`[DEBUG] No MiniMax account data found for ID: ${accountId}`)
      return null
    }

    logger.debug(`[DEBUG] Raw MiniMax account data keys: ${Object.keys(accountData).join(', ')}`)
    ...
    logger.debug(
      `[DEBUG] Final MiniMax account data - name: ${accountData.name}, hasApiUrl: ${!!accountData.apiUrl}, hasApiKey: ${!!accountData.apiKey}, supportedModels: ${JSON.stringify(accountData.supportedModels)}`
    )
  ...
  async updateAccount(accountId, updates) {
    try {
      ...
      // 处理各个字段的更新
      logger.debug(
        `[DEBUG] MiniMax update request received with fields: ${Object.keys(updates).join(', ')}`
      )
      logger.debug(`[DEBUG] MiniMax Updates content: ${JSON.stringify(updates, null, 2)}`)
      ...
      logger.success(`📝 Updated MiniMax account: ${accountId}`)
      return await this.getAccount(accountId)
    } catch (error) {
      logger.error(`❌ Failed to update MiniMax account ${accountId}:`, error)
      throw error
    }
  }
  ...
  async deleteAccount(accountId) {
    try {
      ...
      // 从索引中移除
      await redis.removeFromIndex('minimax_account:index', accountId)

      // 删除账户数据
      const result = await client.del(`${this.ACCOUNT_KEY_PREFIX}${accountId}`)

      if (result === 0) {
        throw new Error('MiniMax Account not found or already deleted')
      }

      logger.success(`🗑️ Deleted MiniMax account: ${accountId}`)
      return { success: true }
    } catch (error) {
      logger.error(`❌ Failed to delete MiniMax account ${accountId}:`, error)
      throw error
    }
  }
  ...
  async markAccountRateLimited(accountId) {
    try {
      ...
      if (account.rateLimitDuration === 0) {
        logger.info(
          `ℹ️ MiniMax account ${account.name} (${accountId}) has rate limiting disabled, skipping rate limit`
        )
        return { success: true, skipped: true }
      }
      ...
      logger.warn(`⏱️ Marked MiniMax account as rate limited: ${account.name} (${accountId})`)
      return { success: true, rateLimitedAt: now }
    } catch (error) {
      logger.error(`❌ Failed to mark MiniMax account as rate limited: ${accountId}`, error)
      throw error
    }
  }
  ...
  async removeAccountRateLimit(accountId) {
    try {
      ...
      if (quotaStoppedAt) {
        ...
        logger.info(
          `ℹ️ MiniMax account ${accountId} rate limit removed but remains stopped due to quota exceeded`
        )
      } else {
        logger.success(`Removed rate limit for MiniMax account: ${accountId}`)
      }
      ...
      logger.error(`❌ Failed to remove rate limit for MiniMax account: ${accountId}`, error)
      throw error
    }
  }
  ...
  async isAccountRateLimited(accountId) {
    try {
      ...
      logger.error(`❌ Failed to check rate limit status for MiniMax account: ${accountId}`, error)
      return false
    }
  }
  ...
  async markAccountOverloaded(accountId) {
    try {
      ...
      if (account.disableAutoProtection === true || account.disableAutoProtection === 'true') {
        ...
        upstreamErrorHelper
          .recordErrorHistory(accountId, 'minimax', 529, 'overload')
          .catch(() => {})
        return { success: true, skipped: true }
      }
      ...
      logger.warn(`🔥 Marked MiniMax account as overloaded: ${account.name} (${accountId})`)
      return { success: true, overloadedAt: now }
    } catch (error) {
      logger.error(`❌ Failed to mark MiniMax account as overloaded: ${accountId}`, error)
      throw error
    }
  }
  ...
  async removeAccountOverload(accountId) {
    try {
      ...
      logger.success(`Removed overload status for MiniMax account: ${accountId}`)
      return { success: true }
    } catch (error) {
      logger.error(`❌ Failed to remove overload status for MiniMax account: ${accountId}`, error)
      throw error
    }
  }
  ...
  async isAccountOverloaded(accountId) {
    try {
      ...
      logger.error(`❌ Failed to check overload status for MiniMax account: ${accountId}`, error)
      return false
    }
  }
  ...
  async markAccountUnauthorized(accountId) {
    try {
      ...
      logger.warn(`🚫 Marked MiniMax account as unauthorized: ${account.name} (${accountId})`)
      return { success: true }
    } catch (error) {
      logger.error(`❌ Failed to mark MiniMax account as unauthorized: ${accountId}`, error)
      throw error
    }
  }
  ...
  async checkQuotaUsage(accountId) {
    try {
      ...
      if (isExceeded) {
        ...
        logger.warn(
          `💰 MiniMax account ${account.name} (${accountId}) quota exceeded: $${dailyUsage.toFixed(2)} / $${dailyQuota.toFixed(2)}`
        )
        ...
        try {
          ...
          logger.warn(
            'Failed to send webhook notification for MiniMax quota exceeded:',
            webhookError
          )
        }
      }
      ...
      logger.error(`❌ Failed to check quota usage for MiniMax account ${accountId}:`, error)
      return false
    }
  }
  ...
  async resetDailyUsage(accountId) {
    try {
      ...
      logger.error(`❌ Failed to reset daily usage for MiniMax account: ${accountId}`, error)
      throw error
    }
  }
  ...
  async isAccountQuotaExceeded(accountId) {
    try {
      ...
      if (isExceeded && !account.quotaStoppedAt) {
        ...
        logger.warn(`💰 MiniMax account ${account.name} (${accountId}) quota exceeded`)
      }

      return isExceeded
    } catch (error) {
      logger.error(`❌ Failed to check quota for MiniMax account ${accountId}:`, error)
      return false
    }
  }
  ...
  async resetAllDailyUsage() {
    try {
      ...
      logger.success(`Reset daily usage for ${resetCount} MiniMax accounts`)
      return { success: true, resetCount }
    } catch (error) {
      logger.error('❌ Failed to reset all MiniMax daily usage:', error)
      throw error
    }
  }

  // 📊 获取MiniMax账户使用统计（含每日费用）
  ...
  async resetAccountStatus(accountId) {
    try {
      ...
      logger.success(`Reset all error status for MiniMax account ${accountId}`)
      ...
      try {
        ...
        logger.warn('Failed to send webhook notification for MiniMax status reset:', webhookError)
      }

      return { success: true, accountId }
    } catch (error) {
      logger.error(`❌ Failed to reset MiniMax account status: ${accountId}`, error)
      throw error
    }
  }
